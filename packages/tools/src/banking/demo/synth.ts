...
const randToken = (rng: () => number, len = 10): string => {
  let s = ''
  for (let i = 0; i < len; i++)
    s += TOKEN_CHARS[Math.floor(rng() * TOKEN_CHARS.length)]
  return s
}
...
const amountDraw = (rng: () => number, guide: DemoGuide): number => {
  const raw = Math.round(
    guide.invoiceAmountMedianCents * Math.exp(gauss(rng) * guide.amountSigma)
  )
...
export const synthDemoData = (guide: DemoGuide, seed: number): DemoData => {
  ...
  const numberPrefixes: DemoNumberPrefix[] = [
    { name: '2026-', template: '2026-' }
  ]
  ...
  const receiptShare =
    guide.invoiceStatusMix.receipt /
    (guide.invoiceStatusMix.bill + guide.invoiceStatusMix.receipt)
  ...
  for (let s = 0; s < guide.settlementCount; s++) {
    const k =
      ...
      Math.floor(
        rng() *
          (guide.settlementPaymentCountRange[1] -
            guide.settlementPaymentCountRange[0] +
            1)
      )
    ...
    for (let j = 0; j < k; j++) {
      ...
      const amount = Math.max(
        1,
        Math.round(inv.amountCents * (0.2 + rng() * 0.8))
      )
      const method: DemoPaymentMethod =
        rng() < guide.methodMix.ideal ? 'ideal' : 'creditcard'
    ...
    for (const p of chunk) {
      if (rng() < guide.refundRate) {
        const refundAmount = Math.max(
          1,
          Math.round(p.amountCents * (0.1 + rng() * 0.9))
        )
    ...
    const fee =
      (guide.settlementFeeFixedCents + guide.settlementFeeRoundingCents) *
      chunk.length
  ...
  const FUZZY_VARIANTS: Array<{
    ...
    {
      clientIdx: 1,
      payer: 'Van Dijck Elektra B.V.',
      remitToken: 'Project nr 2026-108'
    },
    {
      clientIdx: 2,
      payer: 'Bakker en Zonen',
      remitToken: 'Levering bakstenen'
    },
    {
      clientIdx: 3,
      payer: 'J. Jansen Tuinbouw',
      remitToken: 'Onderhoud kas Q3'
    }
  ]
  ...
  const adoptableInvoices = numberedInvoices
    .filter((inv) => inv.status === 'paid')
    .slice(0, 3)
  for (const inv of adoptableInvoices) {
    ...
    const existingPayment = corePayments.find(
      (p) =>
        p.invoiceIndex === numberedInvoices.indexOf(inv) &&
